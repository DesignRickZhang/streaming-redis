...
object RecoverableNetworkWordCount {
  ...
  def createContext(ip: String, port: Int, outputPath: String, checkpointDirectory: String)
    ...
    val wordCounts2 = lines.flatMap(_.split(" ")).map((_,1)).reduceByKey(_+_)
    wordCounts2.foreachRDD((rdd,time)=>{
    	val blacklist2 = WordBlacklist.getInstance(rdd.sparkContext)
    	val droppedWordsCouter2 = DroppedWordsCounter2.getInstance(rdd.sparkContext)
      val counts  = rdd.filter{case (word,count) =>
          if(blacklist2.value.contains(word)){
            droppedWordsCouter2 +=count
            false
          }else{
            true
          }
